...
export interface ViewProfile<DataType extends object, RobloxMetadata = unknown> {
	...
	/**
	 * Stores the Value of ValueObject into the Profile's Data table when the ValueObject changes.
	...
	/**
	 * Stores the value into `Profile.Data` following a path. This is how you now store data into the profile.
	 * @param path The path to the object from the data table. This is a dot separated string.
	 * @param value The value to store.
	 */
...
export interface ProfileStore<T extends object, RobloxMetadata = unknown> {
	 ...
	 *
	 * @example
	 * const profile = ProfileStore.LoadProfile("Player_2312310", (placeId, gameJobId) => {
	 ...
	 *
	 * @example
	 * ProfileStore.LoadProfileAsync("Player_2312310", (placeId, gameJobId) => {
	 * 	// placeId and gameJobId identify the Roblox server that has
	 * 	// this profile currently locked. In rare cases, if the server
	 * 	// crashes, the profile will stay locked until ForceLoaded by
	 * 	// a new session.
	 ...
	 */
	GlobalUpdateProfile(profileKey: string, updateHandler: GlobalUpdateHandler): GlobalUpdates | void;
	GlobalUpdateProfileAsync(profileKey: string, updateHandler: GlobalUpdateHandler): Promise<GlobalUpdates | void>;
